...
class link //create a class ,a type of link
{
 ...
 while (ch != 'x')  //condition : input is not x otherwise break the loop
   { menu();
     ...
     {   int post, data;
         ...
         cout<<"\n Enter the position where you want to insert your data : ";
         cin>>post;
     ...
     else if (ch=='e')
     {
         ...
         cin >> post;
         if (0 < post && post <= num)  //condition for inserting the linked list.
         {
             l_l.delete_at(post);  //calling the delete_at() function
         }
         else {cout<<"Invalid position.";}  //condition when the position entered is not valid for the linked list
     }
     else if (ch=='f')
     {
        cout<<"This is your data :"<<endl;
        l_l.display_l();  //calling  display_l() function
     }
     else if (ch=='g')
     {   int num;
         num = l_l.count_l();  //calling  count_l() funtion
         cout<<"\n There is ";
         cout<<num;
        ...
        {
             cout<<"\n You have entered an invalid key try again.";  //when you enter a invalid character
        }

     cout <<"\n  Thanking you for using this programme.";
     cout <<"\n  If you want to continue press any key and hit 'enter'.";  //run the while loop again
     cout <<"\n  If you want to exit press 'Y' and hit 'enter'. \n -> ";   //break the while loop
     cin >> ch; 
   }
...
void link :: insert_list(int data)
{
     ...
     if (head == NULL)     //condition is onle able for empty list
     {
...
{
    node *pos;  //create a new node type pointer called 'pos'
    pos=head;  //making the address of 'pos' and 'head' same
    if (head==NULL)
    {
    ...
    else
    {
       while (pos != NULL)
      {
         ...
         pos=pos->next; // then it goes to next element 
      }
    }
}
...
int link :: count_l()
{
    ...
    else
    {
        ...
        return i; //return this number where the element is exist
    }
}
